...
export default function EditVehicleForm({ initialData }: { initialData: any }) {
    ...
    const [previewImage, setPreviewImage] = useState<string | null>(initialData?.images?.[0] || null);
    const [selectedType, setSelectedType] = useState<string>(() => {
        if (!initialData?.type) return '';
        const exists = VEHICLE_TYPES.some(t => t.value === initialData.type);
        return exists ? initialData.type : 'otra';
    });
    ...
    const action: (payload: FormData) => void = async (formData) => {
        ...
        try {
            const result = await updateVehicle(initialData.id, formData);
            if (result.error) {
        ...
        <form action={action} className="space-y-8">
            <div className="grid sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input name="brand" required defaultValue={initialData?.brand} className="input-dark w-full" placeholder="Ej: Toyota" />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Modelo</label>
                    <input name="model" required defaultValue={initialData?.model} className="input-dark w-full" placeholder="Ej: Hilux 4x4" />
                </div>
                <div>
                    ...
                    <div className="flex gap-2">
                        ...
                        {selectedType === 'otra' && (
                            <input
                                ...
                                type="text"
                                required
                                defaultValue={initialData?.type && !VEHICLE_TYPES.some(t => t.value === initialData.type) ? initialData.type : ''}
                ...
                <div>
                    ...
                    <input name="year" type="number" required defaultValue={initialData?.year} className="input-dark w-full" placeholder="Ej: 2024" min="2000" max="2100" />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Tarifa por Día (S/)</label>
                    <input name="pricePerDay" type="number" required defaultValue={initialData?.pricePerDay} className="input-dark w-full" placeholder="Ej: 250" min="0" />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Transmisión</label>
                    <select name="transmission" required defaultValue={initialData?.transmission} className="input-dark w-full appearance-none bg-transparent">
                        <option value="">Selecciona...</option>
            ...
            <div className="grid sm:grid-cols-4 gap-6">
                <div>
                    ...
                    <input name="passengers" type="number" required defaultValue={initialData?.passengers} className="input-dark w-full" min="1" max="20" />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Puertas</label>
                    <input name="doors" type="number" required defaultValue={initialData?.doors} className="input-dark w-full" min="2" max="6" />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Maletas</label>
                    <input name="luggage" type="number" required defaultValue={initialData?.luggage} className="input-dark w-full" min="0" max="10" />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Combustible</label>
                    <select name="fuelType" required defaultValue={initialData?.fuelType} className="input-dark w-full appearance-none bg-transparent">
            ...
            <div>
                ...
                <textarea name="description" required defaultValue={initialData?.description} rows={4} className="input-dark w-full resize-none" placeholder="Describe las ventajas de este vehículo para el cliente." />
            </div>

            <div>
                <label className="block text-sm font-medium mb-1.5 text-gray-700 dark:text-gray-300">Foto del Vehículo (Opcional, elige solo si quieres reemplazar la actual)</label>
                <div className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed rounded-xl relative overflow-hidden ${previewImage ? 'border-primary' : 'border-gray-300 dark:border-dark-600'}`}>
                    ...
                    <div className="space-y-1 text-center relative z-10">
                        ...
                        <p className="text-xs text-gray-500">PNG, JPG, WEBP hasta 2MB (se mantendrá la actual si no subes una nueva)</p>
                    </div>
                </div>
            </div>
            ...
            <div className="pt-6 border-t border-gray-200 dark:border-dark-700 flex items-center justify-end gap-4">
                ...
                <button type="submit" disabled={isSubmitting} className="btn-primary flex items-center gap-2 px-8 py-2.5">
                    ...
                    {isSubmitting ? 'Guardando...' : 'Actualizar Vehículo'}
                </button>
            </div>
        </form>
    );
